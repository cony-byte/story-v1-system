...
2. **발행성과 실무 합의**: episode_id **복합키 형식**(`char_x/ep_y`)을 실무자와 지금 합의. 소급은 지옥이다.
   시트 교정본은 준비됨 — `~/Downloads/발행성과_입력시트.csv` (D+7/D+30 예시 2행, notes '예시' 포함이라 엔진 자동 스킵). Drive 입력 시트에서 파일→가져오기→"스프레드시트 바꾸기"로 넣으면 끝.
...
| `~/Downloads/발행성과_입력시트.csv` | **Drive 입력 시트용 최종 교정본** (복합키 + D+7/D+30 예시 2행) |
| `~/Downloads/발행성과 템플릿.csv` | 복합키 교정 1차본 (입력시트.csv로 대체됨) |
